...
public class InnerWeekView extends View {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            switch (mCurrentScrollDirection) {
                ...
                case RIGHT:
                    ...
                    ViewCompat.postInvalidateOnAnimation(InnerWeekView.this);
                    break;
                case VERTICAL:
    ...
    public InnerWeekView(Context context) {
        this(context, null);
    }

    public InnerWeekView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public InnerWeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.InnerWeekView, 0, 0);
        try {
            mFirstDayOfWeek = a.getInteger(R.styleable.InnerWeekView_firstDayOfWeek, mFirstDayOfWeek);
            mHourHeight = a.getDimensionPixelSize(R.styleable.InnerWeekView_hourHeight, mHourHeight);
            mMinHourHeight = a.getDimensionPixelSize(R.styleable.InnerWeekView_minHourHeight, mMinHourHeight);
            mEffectiveMinHourHeight = mMinHourHeight;
            mMaxHourHeight = a.getDimensionPixelSize(R.styleable.InnerWeekView_maxHourHeight, mMaxHourHeight);
            mTextSize = a.getDimensionPixelSize(R.styleable.InnerWeekView_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mTextSize, context.getResources().getDisplayMetrics()));
            mHeaderColumnPadding = a.getDimensionPixelSize(R.styleable.InnerWeekView_headerColumnPadding, mHeaderColumnPadding);
            mColumnGap = a.getDimensionPixelSize(R.styleable.InnerWeekView_columnGap, mColumnGap);
            mHeaderColumnTextColor = a.getColor(R.styleable.InnerWeekView_headerColumnTextColor, mHeaderColumnTextColor);
            mNumberOfVisibleDays = a.getInteger(R.styleable.InnerWeekView_noOfVisibleDays, mNumberOfVisibleDays);
            mShowFirstDayOfWeekFirst = a.getBoolean(R.styleable.InnerWeekView_showFirstDayOfWeekFirst, mShowFirstDayOfWeekFirst);
            mHeaderRowPadding = a.getDimensionPixelSize(R.styleable.InnerWeekView_headerRowPadding, mHeaderRowPadding);
            mHeaderRowBackgroundColor = a.getColor(R.styleable.InnerWeekView_headerRowBackgroundColor, mHeaderRowBackgroundColor);
            mDayBackgroundColor = a.getColor(R.styleable.InnerWeekView_dayBackgroundColor, mDayBackgroundColor);
            mFutureBackgroundColor = a.getColor(R.styleable.InnerWeekView_futureBackgroundColor, mFutureBackgroundColor);
            mPastBackgroundColor = a.getColor(R.styleable.InnerWeekView_pastBackgroundColor, mPastBackgroundColor);
            mFutureWeekendBackgroundColor = a.getColor(R.styleable.InnerWeekView_futureWeekendBackgroundColor, mFutureBackgroundColor); // If not set, use the same color as in the week
            mPastWeekendBackgroundColor = a.getColor(R.styleable.InnerWeekView_pastWeekendBackgroundColor, mPastBackgroundColor);
            mNowLineColor = a.getColor(R.styleable.InnerWeekView_nowLineColor, mNowLineColor);
            mNowLineThickness = a.getDimensionPixelSize(R.styleable.InnerWeekView_nowLineThickness, mNowLineThickness);
            mHourSeparatorColor = a.getColor(R.styleable.InnerWeekView_hourSeparatorColor, mHourSeparatorColor);
            mTodayBackgroundColor = a.getColor(R.styleable.InnerWeekView_todayBackgroundColor, mTodayBackgroundColor);
            mHourSeparatorHeight = a.getDimensionPixelSize(R.styleable.InnerWeekView_hourSeparatorHeight, mHourSeparatorHeight);
            mTodayHeaderTextColor = a.getColor(R.styleable.InnerWeekView_todayHeaderTextColor, mTodayHeaderTextColor);
            mEventTextSize = a.getDimensionPixelSize(R.styleable.InnerWeekView_eventTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mEventTextSize, context.getResources().getDisplayMetrics()));
            mEventTextColor = a.getColor(R.styleable.InnerWeekView_eventTextColor, mEventTextColor);
            mEventPadding = a.getDimensionPixelSize(R.styleable.InnerWeekView_eventPadding, mEventPadding);
            mHeaderColumnBackgroundColor = a.getColor(R.styleable.InnerWeekView_headerColumnBackground, mHeaderColumnBackgroundColor);
            mDayNameLength = a.getInteger(R.styleable.InnerWeekView_dayNameLength, mDayNameLength);
            mOverlappingEventGap = a.getDimensionPixelSize(R.styleable.InnerWeekView_overlappingEventGap, mOverlappingEventGap);
            mEventMarginVertical = a.getDimensionPixelSize(R.styleable.InnerWeekView_eventMarginVertical, mEventMarginVertical);
            mXScrollingSpeed = a.getFloat(R.styleable.InnerWeekView_xScrollingSpeed, mXScrollingSpeed);
            mEventCornerRadius = a.getDimensionPixelSize(R.styleable.InnerWeekView_eventCornerRadius, mEventCornerRadius);
            mShowDistinctPastFutureColor = a.getBoolean(R.styleable.InnerWeekView_showDistinctPastFutureColor, mShowDistinctPastFutureColor);
            mShowDistinctWeekendColor = a.getBoolean(R.styleable.InnerWeekView_showDistinctWeekendColor, mShowDistinctWeekendColor);
            mShowNowLine = a.getBoolean(R.styleable.InnerWeekView_showNowLine, mShowNowLine);
            mHorizontalFlingEnabled = a.getBoolean(R.styleable.InnerWeekView_horizontalFlingEnabled, mHorizontalFlingEnabled);
            mVerticalFlingEnabled = a.getBoolean(R.styleable.InnerWeekView_verticalFlingEnabled, mVerticalFlingEnabled);
            mAllDayEventHeight = a.getDimensionPixelSize(R.styleable.InnerWeekView_allDayEventHeight, mAllDayEventHeight);
            mScrollDuration = a.getInt(R.styleable.InnerWeekView_scrollDuration, mScrollDuration);
        } finally {
            a.recycle();
        }
    ...
    private void initTextTimeWidth() {
        mTimeTextWidth = 0;
        for (int i = earliestScrollableHour; i < latestScrollableHour; i++) {
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = earliestScrollableHour; i < latestScrollableHour; i++) {
            float top = mHeaderHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * (i - earliestScrollableHour) + mHeaderMarginBottom;
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / getNumHours()));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * getNumHours() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * getNumHours() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < getNumHours(); hourNumber++) {
            ...
            for (List<EventRect> column : columns) {
                if (column.size() >= i+1) {
                    ...
                    if(!eventRect.event.isAllDay()) {
                        eventRect.top = (eventRect.event.getStartTime().get(Calendar.HOUR_OF_DAY) - earliestScrollableHour) * 60 + eventRect.event.getStartTime().get(Calendar.MINUTE);
                        eventRect.bottom = (eventRect.event.getEndTime().get(Calendar.HOUR_OF_DAY) - earliestScrollableHour) * 60 + eventRect.event.getEndTime().get(Calendar.MINUTE);
                    }
                    else{
                        eventRect.top = 0;
    ...
    public void setTodayHeaderTextTypeface(Typeface typeface) {
     ...
     * </p>
     * @param length Supported values are {@link com.alamkanak.weekview.InnerWeekView#LENGTH_SHORT} and
     * {@link com.alamkanak.weekview.InnerWeekView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public void setEarliestScrollableHour(int hour) {
        this.earliestScrollableHour = hour;
    }

    public void setLatestScrollableHour(int hour) {
        this.latestScrollableHour = hour;
    }
    ...
    private void goToNearestOrigin(){
        ...
        if (mCurrentOrigin.x - nearestOrigin > getMaxX()) {
    ...
    private int getNumHours() {
        return latestScrollableHour - earliestScrollableHour;
    }
    ...
    public void goToHour(double hour){
        ...
        if (hour > latestScrollableHour)
            verticalOffset = mHourHeight * getNumHours();
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * (hour - earliestScrollableHour));

        if (verticalOffset > mHourHeight * getNumHours() - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * getNumHours() - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
    ...
    public interface EventLongPressListener {
        /**
         * Similar to {@link com.alamkanak.weekview.InnerWeekView.EventClickListener} but with a long press.
         * @param event: event clicked.
         * @param eventRect: view containing the clicked event.
         */
    ...
    public interface EmptyViewLongPressListener {
        /**
         * Similar to {@link com.alamkanak.weekview.InnerWeekView.EmptyViewClickListener} but with long press.
